  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-3 pb-3 sm:px-6 sm:pb-5">
      <div className="composer-shell">
        ...
        {(allowAttachments || remainingCredits !== undefined) && <div className="flex items-center justify-between gap-3 px-3 pb-2 text-[10px] text-[var(--muted)]"><span>{fileError || <><span className="sm:hidden">3 anexos · 12 MB total</span><span className="hidden sm:inline">Até 3 anexos · 5 MB cada · 12 MB no total</span></>}</span>{remainingCredits !== undefined && <span className={`flex shrink-0 items-center gap-1 ${insufficientCredits ? 'text-red-400' : ''}`}><Coins className="h-3 w-3" /><span className="sm:hidden">{cost} · {remainingCredits}</span><span className="hidden sm:inline">Custo {cost} · Restam {remainingCredits}</span></span>}</div>}
      </div>
